...
var jDownloader = new Class({
    initialize: function() {
        var self = this;
        App.addEvent("loadSettings", self.addGetDevicesButton.bind(self));
    },
    addGetDevicesButton: function() {
        var self = this;
        var setting_page = App.getPage("Settings");
        setting_page.addEvent("create", function() {
            var fieldset = setting_page.tabs.downloaders.groups.jdownloader, l = window.location;
            var dropdownObj = fieldset.getElement("[name*=deviceID]");

            self.getDevices = new Element("a.button", {
                text: 'Fetch',
                events: {
                    click: function() {

                        dropdownObj.empty()
                        Api.request("download.jdownloader.fetchDevices", {
                            onComplete: function(json) {
                                Object.each(json, function(value, text) {
                                    var o = document.createElement("option");
                                    o.text = text; o.value = value;
                                    dropdownObj.add(o);
                                    dropdownObj.fireEvent('change')
                                });
                            }
                        });
                    }
                }
            }).inject(fieldset.getElement("[name*=deviceID]"), "before");
        });
    }
});
